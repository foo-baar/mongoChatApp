Mock chat applicaiton created with NodeJs and ReactJs, utilizing socket.io.

App currently as for twitter Id to log-in (with the purpose of gettin AVATAR).

User gets into the General room with a list of other online users.

User can send public messages in the General room or can start a privare conversaition by selecting user from the avaliable user list.

At present app utilises session based storage (in-memory), on page refresh the user session will expire, however online users list in mainatined on the server hence displays all loggen-in users.
